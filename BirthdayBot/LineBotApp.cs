...
namespace BirthdayBot
{
    public class LineBotApp : WebhookApplication
    {
        ...
        // Botの説明定型文
        const string descriptionMessage =
            "------------------------------\n" +
            " ご利用ありがとうございます！\n\n" +
            "‣「X月X日 名前」で登録\n" +
            "‣「X月X日 名前 更新 X月X日\n" +
        ...
        // 本LineBotで使用するAPIクライアントとDBサービスを設定
        ...
        private async Task HandleTextAsync(string replyToken, string userMessage, string userId)
        {
            ...
            if (Regex.IsMatch(userMessage, @"([1-9１-９]|[1１][0-2０-２])月([1-9１-９]|[12１２][0-9０-９]|[3３][01０１])日\s.*\s更新\s([1-9１-９]|[1１][0-2０-２])月([1-9１-９]|[12１２][0-9０-９]|[3３][01０１])日\s.*") || 
                Regex.IsMatch(userMessage, @"([1-9１-９]|[1１][0-2０-２])/([1-9１-９]|[12１２][0-9０-９]|[3３][01０１])\s.*\s更新\s([1-9１-９]|[1１][0-2０-２])/([1-9１-９]|[12１２][0-9０-９]|[3３][01０１])\s.*"))
            {
                // ユーザメッセージ配列から月日をMM/ddの形に変換(MM/ddの場合は空振る)
                var dateTimeStringForSelect = MMddParser(userMessageArray[0]);
                var dateTimeStringForUpdate = MMddParser(userMessageArray[3]);
                ...
                if (updateItem.Count > 0)
                {
                    ...
                    await messagingClient.ReplyMessageAsync(replyToken, "更新しました！");
                }
                else
                {
                    await messagingClient.ReplyMessageAsync(replyToken, "更新に失敗してしまいました！\n入力いただいた誕生日データはまだ登録されていないようです！");
                }
            }
            else if (Regex.IsMatch(userMessage, @"([1-9１-９]|[1１][0-2０-２])月([1-9１-９]|[12１２][0-9０-９]|[3３][01０１])日\s.*\s削除") || 
                     Regex.IsMatch(userMessage, @"([1-9１-９]|[1１][0-2０-２])/([1-9１-９]|[12１２][0-9０-９]|[3３][01０１])\s.*\s削除"))
            {
                // ユーザメッセージ配列から月日をMM/ddの形に変換(MM/ddの場合は空振る)
                var dateTimeStringForDelete = MMddParser(userMessageArray[0]);
                ...
                {
                    await messagingClient.ReplyMessageAsync(replyToken, "削除しました！");
                }
                else
                {
                    await messagingClient.ReplyMessageAsync(replyToken, "削除に失敗してしまいました！\n入力いただいた誕生日データはまだ登録されていないようです！");
                }
            }
            else if (Regex.IsMatch(userMessage, @"([1-9１-９]|[1１][0-2０-２])月([1-9１-９]|[12１２][0-9０-９]|[3３][01０１])日\s.*") ||
                     Regex.IsMatch(userMessage, @"([1-9１-９]|[1１][0-2０-２])/([1-9１-９]|[12１２][0-9０-９]|[3３][01０１])\s.*"))
            {
                ...
                Item item = new Item
                {
                    ...
                    Birthday = MMddParser(userMessageArray[0])
                };
                ...
                {
                    await messagingClient.ReplyMessageAsync(replyToken, "登録件数が限度の50件に達しているようです！");
                }
                else if (selectNewItemResultList.Count == 0)
                {
                    // 登録
                    await this.cosmosDbService.AddItemAsync(item);
                    await messagingClient.ReplyMessageAsync(replyToken, "登録しました！");
                }
                else
                {
                    await messagingClient.ReplyMessageAsync(replyToken, "入力いただいた誕生日データは既に登録されているようです！");
                }
            }
            else if (userMessage == "一覧")
            {
                ...
                if (selectAllItemResultList.Count != 0)
                {
                    ...
                    await messagingClient.ReplyMessageAsync(replyToken, string.Join("\n", userRegisteredItemList) + "\n計" + userRegisteredItemList.Count +"件が登録されています！");
                }
                else
                {
                    await messagingClient.ReplyMessageAsync(replyToken, "まだ誕生日データがないようです！");
                }
            }
            else
            {
                // Bot操作のキーワードに該当しなかった場合は説明定型文をリプライ
                await messagingClient.ReplyMessageAsync(replyToken, descriptionMessage);
            }
        }
        ...
        {
            // Botの使い方をリプライ
            await this.messagingClient.ReplyMessageAsync(ev.ReplyToken, descriptionMessage);
        }
